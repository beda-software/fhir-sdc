...
async def get_historic_version(resource, history_id):
    ...
    historic_resource = await client._fetch_resource(
            '{0}/{1}/_history/{2}'.format(
                resource.resource_type, resource.id, history_id))
    if historic_resource:
        return client.resource(
            historic_resource['resourceType'],
            **historic_resource)
...
async def aidbox_query(name, **params):
    ...
    return (await client._fetch_resource(
        '$query/{0}'.format(name), params=params)).get('data', None)
...
def make_bundle(resources, type_='searchset', **kwargs):
    ...
    for resource in resources:
        serialized_resource = resource.serialize() \
            if isinstance(resource, AsyncAidboxResource) else resource
