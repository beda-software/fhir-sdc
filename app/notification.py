    ...
    if action == 'create':
        ...
        if notification_type == 'email':
            layout = await sdk.client.resources('NotificationTemplate').get(
                id='email-layout')
            body = transform(jinja_env.from_string(layout['template']).render({
                'body': body, **payload}))
        elif notification_type == 'sms':
            pass
        else:
            raise Exception('Notification type `{}` is not supported'.format(
                notification_type))
        ...
        if not send_fn:
            logging.warning(
                'Unhandled notification for provider {}'.format(provider))
            return
