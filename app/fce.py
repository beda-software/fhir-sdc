...
def get_updated_properties_from_item(item):
    ...
    initial_expression = find_extension(item, 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-initialExpression')
    if initial_expression is not None:
        initial_expression = initial_expression['valueExpression']
    item_control = find_extension(item, 'http://hl7.org/fhir/StructureDefinition/questionnaire-itemControl')
    ...
    if item_type == 'choice':
        ...
        updated_properties['answerOption'] = [
            {"value": {"string": option["valueString"]}} if "valueString" in option else
            {
                "value": {
                    "Coding": {
            ...
            for option in answer_option if "valueCoding" in option or "valueString" in option
        ]

        adjust_last_to_right = find_extension(item, 'https://beda.software/fhir-emr-questionnaire/adjust-last-to-right')
    ...
    if item_type == 'decimal':
        slider_start = find_extension(item, 'https://beda.software/fhir-emr-questionnaire/slider-start')
        if slider_start is not None:
            updated_properties['start'] = slider_start['valueInteger']
        slider_stop = find_extension(item, 'https://beda.software/fhir-emr-questionnaire/slider-stop')
        ...
        stop_label = find_extension(item, 'https://beda.software/fhir-emr-questionnaire/slider-stop-label')
        if stop_label is not None:
            updated_properties['stopLabel'] = stop_label['valueString']
        slider_step_value = find_extension(item, 'http://hl7.org/fhir/StructureDefinition/questionnaire-sliderStepValue')
        if slider_step_value is not None:
            updated_properties['sliderStepValue'] = slider_step_value['valueInteger']

    if item_type == 'integer':
        calculated_expression = find_extension(item, 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-calculatedExpression')
    ...
    if item_type == 'text':
        ...
        enable_when_expression = find_extension(item, 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-enableWhenExpression')
    ...
    if item_type == 'group':
        if "item" in item:
            for nested_item in item["item"]:
                unit = find_extension(nested_item, 'http://hl7.org/fhir/StructureDefinition/questionnaire-unit')
                if unit is not None:
    ...
    if item_type == 'reference':
        choice_column_extension = find_extension(item, 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-choiceColumn')
        if choice_column_extension is not None:
            choice_column_extension = choice_column_extension['extension']
            for_display = next((obj["valueBoolean"] for obj in choice_column_extension if obj["url"] == "forDisplay"), None)
            path = next((obj["valueString"] for obj in choice_column_extension if obj["url"] == "path"), None)
            choice_column_array = [{"forDisplay": for_display or False, "path": path}]
            updated_properties['choiceColumn'] = choice_column_array

        answer_expression = find_extension(item, 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-answerExpression')
        if answer_expression is not None:
            updated_properties['answerExpression'] = answer_expression['valueExpression']

        reference_resource = find_extension(item, 'http://hl7.org/fhir/StructureDefinition/questionnaire-referenceResource')
...
def convert_item_properties(item):
    ...
    if new_item.get('item'):
        new_item['item'] = [convert_item_properties(nested_item) for nested_item in new_item['item']]
...
def check_fhir_questionnaire_profile(fhir_questionnaire):
    if (
        not (
            len(fhir_questionnaire.get('meta', {}).get('profile', [])) == 1 and
            fhir_questionnaire.get('meta', {}).get('profile', [])[0] == 'https://beda.software/beda-emr-questionnaire'
        )
    ):
...
def get_created_at(fhir_questionnaire):
    meta_extension = next((ext for ext in fhir_questionnaire.get('meta', {}).get('extension', []) if ext.get('url') == 'ex:createdAt'), None)
...
def process_launch_context(fhir_questionnaire):
    ...
    launch_context_extensions = [ext for ext in launch_context_extensions if ext.get(
    ...
    for launch_context_extension in launch_context_extensions:
        name_extension = next((ext for ext in launch_context_extension.get('extension', []) if ext.get('url') == 'name'), None)
        type_extension = next((ext for ext in launch_context_extension.get('extension', []) if ext.get('url') == 'type'), None)
        description_extension = next((ext for ext in launch_context_extension.get('extension', []) if ext.get('url') == 'description'), None)
...
def processMapping(fhirQuestionnaire):
    mapperExtensions = list(filter(lambda ext: ext.get('url') == 'http://beda.software/fhir-extensions/questionnaire-mapper',
                            fhirQuestionnaire.get('extension', [])))

    if not mapperExtensions:
        return None

    return list(map(lambda mapperExtension: {'id': mapperExtension.get('valueReference', {}).get('reference', '').split('/')[1], 'resourceType': 'Mapping'}, mapperExtensions))
...
def processItems(fhirQuestionnaire):
    return list(map(process_item, fhirQuestionnaire.get('item', []))) if 'item' in fhirQuestionnaire else None
...
def processAnswerToFCE(itemList):
    ...
    for item in itemList:
        if item.get('answer') and item.get('answer')[0].get('valueString'):
            ...
            item['answer'][0]['value'] = {
                'time': item['answer'][0].get('valueTime')
            }
...
def processAnswerToFHIR(itemList):
    if not itemList:
        return
    for item in itemList:
        if item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('string'):
            item['answer'][0]['valueString'] = item['answer'][0]['value']['string']
            del item['answer'][0]['value']
        elif item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('integer'):
            item['answer'][0]['valueInteger'] = item['answer'][0]['value']['integer']
            del item['answer'][0]['value']
        elif item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('boolean'):
            item['answer'][0]['valueBoolean'] = item['answer'][0]['value']['boolean']
            del item['answer'][0]['value']
        elif item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('Coding'):
            item['answer'][0]['valueCoding'] = item['answer'][0]['value']['Coding']
            del item['answer'][0]['value']
        elif item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('date'):
            item['answer'][0]['valueDate'] = item['answer'][0]['value']['date']
            del item['answer'][0]['value']
        elif item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('dateTime'):
            item['answer'][0]['valueDateTime'] = item['answer'][0]['value']['dateTime']
            del item['answer'][0]['value']
        elif item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('Reference'):
            ...
            del item['answer'][0]['value']
        elif item.get('answer') and item.get('answer')[0].get('value') and item.get('answer')[0]['value'].get('time'):
...
def processReferenceToFHIR(fceQR):
    if fceQR.get("encounter", {}).get("resourceType") and fceQR["encounter"].get("id"):
        fceQR["encounter"]["reference"] = f"{fceQR['encounter']['resourceType']}/{fceQR['encounter']['id']}"
...
def process_items_to_fhir(items):
    ...
    for item in items:
        ...
        if item.get('initial'):
            item['initial'] = [
                {'valueBoolean': entry['value']['boolean']} if 'boolean' in entry['value'] else {'valueCoding': entry['value']['Coding']} for entry in item['initial']
            ]
        ...
        if item.get('choiceColumn'):
            ...
            for column in item['choiceColumn']:
                ...
                if isinstance(column.get('forDisplay'), bool):
                    extension['extension'].append({'url': 'forDisplay', 'valueBoolean': column['forDisplay']})
...
def processExtensionsToFHIR(questionnaire):
    ...
    if questionnaire.get('mapping'):
        mapping_extension = {
            ...
            'valueReference': {
                'reference': f"Mapping/{questionnaire.get('mapping')[0].get('id')}"
            }
        }
        questionnaire['extension'] = questionnaire.get('extension', [])
